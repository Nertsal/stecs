...
impl<K: slotmap::Key, T> Default for Dense<T, K> {
    fn default() -> Self {
        Self(Default::default())
    }
}

impl<K: slotmap::Key, T> Storage<T> for Dense<T, K> {
    type Family = DenseFamily<K>;
    type Id = K;
...
impl<K: slotmap::Key> StorageFamily for DenseFamily<K> {
    type Id = K;
    type Storage<T> = Dense<T, K>;
    type Generator = DenseIdGenerator<K>;
}

impl<K: slotmap::Key, T: SplitFields<DenseFamily<K>>> StructOfAble for Dense<T, K> {
    type Struct = T;
    type Family = DenseFamily<K>;
}
